...
public class OCPerlinNoise2D {
	...
	public OCPerlinNoise2D(float scale) {
		this.scale = scale;
		offset = new Vector2( Random.Range(-100f, 100f), Random.Range(-100f, 100f) );
	}
	
	public OCPerlinNoise2D SetPersistence(float persistence) {
		this.persistence = persistence;
		return this;
	}
	
	public OCPerlinNoise2D SetOctaves(int octaves) {
...
public class NoiseArray2D {
	...
	private OCPerlinNoise2D noise;
	private float[,] map = new float[OpenCog.Map.OCChunk.SIZE_X+2, OpenCog.Map.OCChunk.SIZE_Z+2];
	private Vector2i offset;
	
	public NoiseArray2D(float scale) {
		noise = new OCPerlinNoise2D(scale);
	}
