...
[Serializable]
	...
	public class OCPerceptionCollector : OCSingletonMonoBehaviour<OCPerceptionCollector>
	{
		...
		public void Start()
		{
			...
			OCLogger.Normal(gameObject.name + " is started.");
		}
		...
		public void PerceiveWorld()
		{
			...
			for(int iNPC = 0; iNPC < npcArray.Length; iNPC++)
			{
				...
				if(this.BuildMapInfo(npcObject))
				{
					...
					OCLogger.Normal("Added NPC with ID '" + npcObject.GetInstanceID() + "' to updatedObjects");
				} else
				{
					//OCLogger.Normal ("NPC with ID '" + npcObject.GetInstanceID() + "' has not changed, so will not be added to updatedObjects");
				}	
			...
			for(int iAGI = 0; iAGI < agiArray.Length; iAGI++)
			{
				...
				if(this.BuildMapInfo(agiObject))
				{
					...
					//OCLogger.Normal ("Added AGI with ID '" + agiObject.GetInstanceID() + "' to updatedObjects");
				} else
				{
					//OCLogger.Normal ("AGI with ID '" + agiObject.GetInstanceID() + "' has not changed, so will not be added to updatedObjects");
				}	
			}
	...
	//			}
			...
			for(int iBattery = 0; iBattery < batteryArray.Length; iBattery++)
			{
				...
				if(this.BuildMapInfo(batteryObject))
				{
					...
					OCLogger.Normal("Added Battery with ID '" + batteryObject.GetInstanceID() + "' to updatedObjects");
				} else
				{
					//OCLogger.Normal ("Battery with ID '" + batteryObject.GetInstanceID() + "' has not changed, so will not be added to updatedObjects");
				}	
			...
			for(int iHearth = 0; iHearth < hearthArray.Length; iHearth++)
			{
				...
				if(this.BuildMapInfo(hearthObject))
				{
					...
					OCLogger.Normal("Added Hearth with ID '" + hearthObject.GetInstanceID() + "' to updatedObjects");
				} else
				{
					//OCLogger.Normal ("Battery with ID '" + batteryObject.GetInstanceID() + "' has not changed, so will not be added to updatedObjects");
				}	
	...
	//			}
			...
			//OCLogger.Normal ("CacheIDList contains these ID's: " + strCacheIdList );
				
				
			foreach(int oid in cacheIdList)
			{
				//OCLogger.Normal ("Now we're checking _mapInfoCache for the ID " + oid);
				if(!_mapInfoCacheStatus[oid])
				{
					OCLogger.Normal("We didn't find the ID " + oid);
				...
				//else
				//OCLogger.Normal ("We found the ID " + oid);
			}
			...
			{
				//OCLogger.Normal ("PerceptionCollector: global map info has been updated");
		...
		{
			OCLogger.Fine("In changeChunkStatus...");
	...
	//			uint blockZ = (uint)(hitPoint.Z % worldData.ChunkBlockDepth);
				...
				if(v3iBatteryPosition == batteryCreationPoint)
				{
					...
					if(!_mapInfoCache.ContainsKey(batteryObject.GetInstanceID()))
					{
						if(this.BuildMapInfo(batteryObject))
						{
							...
							OCLogger.Normal("I can see a new battery! Its ID is " + batteryObject.GetInstanceID());
						}
					}
				}
			}
			...
			{
				OCLogger.Normal("mapInfo == null, nothing to report.");	
			}
		}
	...
	//			uint blockZ = (uint)(hitPoint.Z % worldData.ChunkBlockDepth);
			...
			for(int iBattery = 0; iBattery < batteryArray.Length; iBattery++)
			{
				...
				{
					//OCLogger.Normal ("We'll be searching the _mapInfoCache for an objeject with key '" + batteryObject.GetInstanceID() + "'");
						
	//					foreach (KeyValuePair<int, OpenCog.Embodiment.OCObjectMapInfo> pair in _mapInfoCache)
	//					{
	//						OCLogger.Normal ("In any case, there is a key '" + pair.Key + "' in it.");	
	//					}
	...
	//		}``
		...
		public IEnumerator PerceiveTerrain()
		{
			...
			if(map == null)
			{
				OCLogger.Normal("OCPerceptionCollector::PerceiveTerrain: map == null");
			}
					...
					for(int z = map.Chunks.GetMinZ(); z < map.Chunks.GetMaxZ(); ++z)
					{
						...
						if(chunk != null)
						{
							if(!chunk.IsEmpty)
							{
								...
								OCLogger.Normal("Perceiving Chunk at position [" + viChunkPosition.x + ", " + viChunkPosition.y + ", " + viChunkPosition.z + "].");
								...
								OCLogger.Normal("   Processing blocks from chunk [" + viChunkStartingCorner.x + ", " + viChunkStartingCorner.y + ", " + viChunkStartingCorner.z + "].");
								OCLogger.Normal("   to [" + viChunkEndingCorner.x + ", " + viChunkEndingCorner.y + ", " + viChunkEndingCorner.z + "].");
									...
									for(int iGlobalY = viChunkStartingCorner.y; iGlobalY <= viChunkEndingCorner.y; iGlobalY++)
									{
										for(int iGlobalZ = viChunkStartingCorner.z; iGlobalZ <= viChunkEndingCorner.z; iGlobalZ++)
										{
											...
											if((!globalBlock.IsEmpty()) && (globalBlock.block.GetName().ToLower() != "battery") && (iGlobalY > _floorHeight))
											{
												...
												{
													OCLogger.Normal("Sending terrain info...");
											...
											if(blocksProcessed % blocksPerDebugEntry == 0)
											{
												...
												OCLogger.Normal("Processed " + blocksPerDebugEntry + " blocks (" + (blocksProcessed - blocksPerDebugEntry) + " - " + blocksProcessed + ") in " + dtProcessingTick.Subtract(dtStartProcessing).TotalMilliseconds + " milliseconds. " + emptyBlocksProcessed + " were empty.");
			...
			OCLogger.Normal("Let's check if there are any blocks left to report in our terrainMapinfoList...");
			if(terrainMapinfoList.Count > 0)
			{
				OCLogger.Normal("   Yep, looks like there are...");
				_connector.SendTerrainInfoMessage(terrainMapinfoList, ! _hasPerceivedTerrainForFirstTime);
				OCLogger.Normal("   Ok, all blocks have been sent now...");
				terrainMapinfoList.Clear();
			} else
			{
				OCLogger.Normal("   Nope, looks like we already sent everything!");	
			}
			...
			if(!_hasPerceivedTerrainForFirstTime)
			{
				...
				OCLogger.Normal("Time to send the 'finished perceiving terrain' message!");
				_connector.SendFinishPerceptTerrain();
				_hasPerceivedTerrainForFirstTime = true;
			} else
			{
				OCLogger.Normal("That's weird...it thinks _hasPerceivedTerrainForFirstTime is true already...");	
			}
				
			OCLogger.Normal("Finished perceiving terrain, total time taken: " + System.DateTime.Now.Subtract(dtStartPerceptTerrain).TotalSeconds);
		...
		{
			//OCLogger.Normal ("OCPerceptionCollector::PerceiveStateChanges");
			...
			foreach(OpenCog.Embodiment.OCStateChangesRegister.StateInfo stateInfo in OpenCog.Embodiment.OCStateChangesRegister.StateList)
			{
				...
				{
					type = "Enum";
					OCLogger.Normal("0.00001 BitCoins says this never happens!! IT'S A STRING FFS WAARHEHFSHJKSJKSH!");
				}
