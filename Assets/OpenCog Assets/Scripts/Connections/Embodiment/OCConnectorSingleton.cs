...
//			SendAvatarSignalsAndTick(basicFactorMap);
	...
	void FixedUpdate()
	{
		...
		{
			//OCLogger.Normal ("OCConnectorSingleton::FixedUpdate sending messages at " + System.DateTime.Now.ToString ("HH:mm:ss.fff"));
			SendMessages();
	...
	public void SendMessages()
	{
		if(!_isInitialized)
		{
			//OCLogger.Normal ("OCConnectorSingleton::SendMessages: not initialized!!");
			return;
		} else
		{
			//OCLogger.Normal ("OCConnectorSingletong::SendMessages: initialized!");
		}
		
		if(_messagesToSend.Count > 0)
		{
			//OCLogger.Normal ("OCConnectorSingleton::SendMessages: " + _messagesToSend.Count + " to send.");
			...
			{
				//OCLogger.Normal ("Processing message " + iMessageIndex + " of " + localMessagesToSend.Count + "...");
				...
				if(message != null)
				{
					...
					{
						OCLogger.Normal("Router not available. Discarding message to '" +
							message.TargetID + "' of type '" + message.Type + "': " + message.ToString());
						continue;
					}
					if(!IsElementAvailable(message.TargetID))
					{
						OCLogger.Normal("Destination not available. Discarding message to '" +
							message.TargetID + "' of type '" + message.Type + "': " + message.ToString());
						continue;
					}
					...
					if(message.Type != OCMessage.MessageType.TICK && !message.MessageContent.Contains("physiology"))
					{
						if(sendResult)
						{
							OCLogger.Normal("Message from '" + message.SourceID + "' to '" + message.TargetID + "' of type '" + message.Type + "': " + message.ToString());
						} else
						{
							OCLogger.Normal("Error sending message from '" + message.SourceID + "' to '" +
								message.TargetID + "' type '" + message.Type + "': " + message.ToString());
						}
					}
				} else
				{
					OCLogger.Normal("A null message...great...");
				}
...
//		else
//			OCLogger.Normal ("OCConnectorSingleton::SendMessages: Nothing to send.");
...
//		        _globalStartPositionY = 0;
		...
		OCLogger.Normal("OCConnectorSingleton reports: OpenCog will receive a world with globalStartPosition [" + _globalStartPositionX + ", " + _globalStartPositionY + ", " + _globalStartPositionZ + "] and blockCounts [" + _blockCountX + ", " + _blockCountY + ", " + _blockCountZ + "].");
	...
	{
		OCLogger.Normal("Attempting to connect OAC...");
		if(_firstRun)
		{
			_firstRun = false;
			...
			{
				OCLogger.Normal("Connection established.");
			}
			...
			do
			{
				...
				OCLogger.Fine("Waiting for spawner...");
			...
			{
				OCLogger.Normal("Spawner obtained, OAC can be launched.");
			}
   ...
   */
	...
	{
//		OCLogger.Normal("OCConnectorSingleton::ProcessNextMessage: " + message.ToString());
		...
		if(message.ToString().StartsWith(SUCCESS_LOAD))
		{
			...
			OCLogger.Fine("Successfully loaded '" + neId + "'.");
		...
		if(message.ToString().StartsWith(SUCCESS_UNLOAD))
		{
			...
			string neId = tokens[2];
			OCLogger.Normal("!!! Successfully unloaded '" + neId + "'.");
			_isInitialized = false;//_isLoaded = false;
		} else
		{
//			OCLogger.Normal ("Processing an interesting message type!");
...
//      //OCLogger.Warn("Action plan " + this.currentPlanId + " sequence " + action.Sequence + " status sent.");
//      lock (_actionsList)
//      {
...
//  }
	...
	public void UpdateActionStatuses()
	{
		...
		OCLogger.Normal("Queuing message to update action statuses.");
		string timestamp = GetCurrentTimestamp();
...
//        //OCLogger.Warn("skipping action result from " + ar.avatar);
//        return;
//    }
		...
		{
			OCLogger.Warn("Error target type: " + targetType + " in action: " + step.Arguments.ActionName);
		}
...
//    }
	...
	public void HandleObjectAppearOrDisappear(string objectID, string objectType, bool isAppear)
	{
		if(isAppear)
		{
			OCLogger.Normal("Reporting appearance of object with ID '" + objectID + "' of type '" + objectType + "'.");
		} else
		{
			OCLogger.Normal("Reporting disappearance of object with ID '" + objectID + "' of type '" + objectType + "'.");
		}
   ...
   */
	public void SendMapInfoMessage(List<OCObjectMapInfo> newMapInfoSeq, bool isFirstTimePerceptMapObjects= false)
	{
		...
		foreach(OCObjectMapInfo objMapInfo in localMapInfo)
		{
			...
			{
				OCLogger.Normal("Damn, objectMapInfo with ID == null....");
			} else
			{
				if(objMapInfo.ID.Equals(_brainID))
				{
					//OCLogger.Normal ("OCConnectorSingleton::SendMapInfoMessage: objMapInfo.ID.Equals(_brainID), moving its objectmapinfo to first position...");
...
//			else
//				OCLogger.Normal ("OCConnectorSingleton::SendMapInfoMessage: objMapInfo.ID.Equals(_brainID) == false");
			}
		} // foreach
...
//		mapInfo.SetAttribute("global-position-offset-z", _blockCountZ.ToString());
   ...
   */
	...
	{
//		OCLogger.Normal ("OCConnectorSingleton::ParseEmotionalFeelingElement");
		//string avatarId = element.GetAttribute(OCEmbodimentXMLTags.ENTITY_ID_ATTRIBUTE);
...
//        emotionalExpression.showEmotionExpression(this.FeelingValueMap);
	...
	{
		//OCLogger.Normal ("OCConnectorSingleton::ParseDOMDocument");
		...
		if(!wrotePlan)
		{
			...
			{
				OCLogger.Normal("A plan! " + stringWriter.ToString());	
				wrotePlan = true;	
			}
		}
		...
		for(int i = 0; i < list.Count; i++)
		{
			OCLogger.Normal("OCConnectorSingleton::ParseDOMDocument: ParseActionPlanElement");
		...
		{
			//OCLogger.Normal ("OCConnectorSingleton::ParseDOMDocument: ParseEmotionalFeelingElement");
		...
		{
			OCLogger.Normal("OCConnectorSingleton::ParseDOMDocument: ParsePsiDemandElement");
		...
		{
			OCLogger.Normal("OCConnectorSingleton::ParseDOMDocument: ParseSingleActionElement");
...
//						}
					...
					vectorGameObject.transform.parent = _map.WaypointsSceneObject.transform;
						...
						// OCLogger.Normal ("A '" + actionName + "' command told me to go to [" + x + ", " + y + ", " + z + "]");
					...
					textMesh.text = sequence.ToString();
						//OCLogger.Normal ("A '" + actionName + "' command (planID = " +  _currentPlanId + ", sequence = " + sequence + " told me to go to [" + x + ", " + z + ", " + y + "]");
				...
				case "entity":
					...
					if(actionName == "grab" || actionName == "eat")
					{
						...
						if(actionArguments.EndTarget != null)
						{
							...
							if(actionName == "grab")
							{
								//OCLogger.Normal ("A 'grab' command (planID = " +  _currentPlanId + ", sequence = " + sequence + " told me to grab an object with ID " + entityID);
							...
							{
								//OCLogger.Normal ("An 'eat' command (planID = " +  _currentPlanId + ", sequence = " + sequence + " told me to eat an object with ID " + entityID);
						...
						} else
						{
							...
							OCLogger.Normal("Received a grab or eat command, but couldn't find the battery in Unity!");
						}
...
//						XmlElement stringElement = (XmlElement)stringParameterChildren.Item (0);
					...
					if(actionName == "say")
					{
						...
						{
							OCLogger.Normal("Robot say: " + toSay + "(actionPlan = " + _currentPlanId + ", sequence = " + sequence);
	...
	public void SendAvatarSignalsAndTick(Dictionary<string, double> physiologicalInfo)
	{
		if(_isEstablished)
		{
			//OCLogger.Normal ("OCConnectorSingleton::SendAvatarSignalsAndTick: _isEstablished -> Sending tick message and phys info.");
			string timestamp = GetCurrentTimestamp();
			XmlDocument doc = new XmlDocument();
			XmlElement root = MakeXMLElementRoot(doc);
...
//						OCLogger.Normal ("Its the tick!");
		...
		{
			//OCLogger.Normal ("OCConnectorSingleton::SendAvatarSignalsAndTick: !isEstablished -> Not sending a tick message / phys info.");	
		}
	}
...
//		
//		OCLogger.Normal ("Send ActionStatus message: " + stringWriter.ToString());	
		...
		//OCLogger.Normal ("Queued message to report '" + ((success ? "done (success)" : "error") + "' on action '" + actionName + "' (planID = " + planId + ", sequence = " + sequence.ToString () + ")"));
	}
...
//		doc.WriteTo(xmlTextWriter);
//		
//		OCLogger.Normal ("Send ActionPlanStatus message: " + stringWriter.ToString());	
		...
		OCLogger.Normal("Queued message to report '" + ((success ? "done (success)" : "error") + "' on actionPlan " + planId));
	}
