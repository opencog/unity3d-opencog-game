...
[Serializable]
	...
	public class OCUnitTests : OpenCog.OCSingletonMonoBehaviour<OCUnitTests>
	{
		...
		public bool TestInitialization()
		{
			...
			for(uint i = 0; i < numTests; i++)
			{
				...
				foreach(object o in needsInit[i])
				{
					...
					if(o == null)
					{
						OCLogger.Error("Disabling test " + testConfigNames[i] + "owed to null parameters. Check the inspector. Unit test will automatically fail."); 
		...
		public IEnumerator RunTests()
		{
			...
			if(!result)
			{
				OCLogger.Normal("One or more of the Unit Tests Failed");
			}
		...
		protected IEnumerator TestEmbodiment()
		{
			...
			if(didConnect == false)
			{
				OCLogger.Normal("Testing the Embodiment, Failed");
				results[(int)TestTypes.EMBODIMENT] = false;
				result = false;
			} else
			{
				OCLogger.Normal("Testing the Embodiment, Succeeded");
			}
			yield break;
		...
		public void Start()
		{
			...
			OCLogger.Normal ("Running Unit Tests");
