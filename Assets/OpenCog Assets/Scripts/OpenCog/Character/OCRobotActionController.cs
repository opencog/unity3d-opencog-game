...
//		if(distanceVec.y < -1.0f + 0.5f && distanceVec.y > -1.0f - 0.5f)
				if (distanceVec.sqrMagnitude < 2.25f) {
					...
					map.SetBlockAndRecompute (new OpenCog.Map.OCBlockData (), TargetBlockPos);
					...
					for (int cx=chunks.GetMinX(); cx<chunks.GetMaxX(); ++cx) {
						for (int cy=chunks.GetMinY(); cy<chunks.GetMaxY(); ++cy) {
							for (int cz=chunks.GetMinZ(); cz<chunks.GetMaxZ(); ++cz) {
								...
								OpenCog.Map.OCChunk chunk = chunks.SafeGet (chunkPos);
								if (chunk != null) {
									for (int z=0; z<OpenCog.Map.OCChunk.SIZE_Z; z++) {
										for (int x=0; x<OpenCog.Map.OCChunk.SIZE_X; x++) {
											for (int y=0; y<OpenCog.Map.OCChunk.SIZE_Y; y++) {
												...
												OpenCog.Map.OCBlockData blockData = chunk.GetBlock (localPos);
												Vector3i candidatePos = OpenCog.Map.OCChunk.ToWorldPosition (chunk.GetPosition (), localPos);
